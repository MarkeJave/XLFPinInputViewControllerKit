...
@implementation XLFPinNumPadView
...
- (void)setupViews
{
    ...
    for (NSUInteger row = 0; row < 4; row++)
    {
        ...
        for (NSUInteger col = 0; col < 3; col++)
        {
            ...
            XLFPinNumButton *button = [[XLFPinNumButton alloc] initWithNumber:number
    ...
    NSDictionary *metrics = @{ @"rowHeight" : @([XLFPinNumButton diameter]),
                               @"vPadding" : @(_vPadding) };
...
{
    return CGSizeMake(3.0f * [XLFPinNumButton diameter] + 2.0f * self.hPadding,
                      4.0f * [XLFPinNumButton diameter] + 3.0f * self.vPadding);
}
...
{
    [self.delegate pinNumPadView:self numberTapped:[(XLFPinNumButton *)sender number]];
}
